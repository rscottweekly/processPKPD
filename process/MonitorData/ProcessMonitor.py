...
def convert_monitordata(coding_info, reprocess):
    ...
    for index, patient in coding_info.iterrows():
        ...
        filename = os.path.join(settings.dir_monitor_raw,  patient.UniqueID + ".xls")

        #don't reprocess anything unless reprocess is set
        if os.path.exists(os.path.join(settings.prod_dir_monitor, patient.UniqueID+".median.csv")) and not reprocess:
            continue
        ...
        df_min_median.to_csv(os.path.join(settings.prod_dir_monitor,patient.UniqueID+'.median.csv'),na_rep="na")
        df_min_mean.to_csv(os.path.join(settings.prod_dir_monitor,patient.UniqueID+'.mean.csv'),na_rep="na")
